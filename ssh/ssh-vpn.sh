...
apt-get remove --purge ufw firewalld -y # disable firewalld
...
<html>
<kepala>
<meta http-equiv="REFRESH" content="0;url=https://wa.me/254778216971">
</kepala>
<tubuh>
<p>DARKANON</p>
</tubuh>
...
detect_os() {
  if [[ -f /etc/os-release ]]; then
    ...
    echo "$ID $VERSION_ID"  # Returns OS ID and version
  else
    echo "Unknown"
  fi
}
...
if [[ "$os_version" =~ "ubuntu 24" ]]; then 
  ...
  echo "Operating system or version not recognized. Exiting..."
  exit 1
fi
...
set_permissions() {
  for log_file in "${LOG_FILES[@]}"; do
    if [[ -f "$log_file" ]]; then
      echo "Setting permissions and ownership for $log_file..."
      chmod 640 "$log_file"
      chown syslog:adm "$log_file"  # Give ownership to syslog so it can write logs
    else
...
# Check if dropbear configuration already exists
check_dropbear_log() {
  grep -q 'if \$programname == "dropbear"' "$RSYSLOG_FILE"
}

# Function to add dropbear configuration
add_dropbear_log() {
  echo "Adding Dropbear configuration to $RSYSLOG_FILE..."
  sudo bash -c "echo -e 'if \$programname == \"dropbear\" then /var/log/auth.log\n& stop' >> $RSYSLOG_FILE"
  systemctl restart rsyslog
  echo "Dropbear configuration added and Rsyslog restarted."
}

if check_dropbear_log; then
  echo "Dropbear configuration already exists, no changes made."
else
  add_dropbear_log
fi
...
# Replace placeholder with IP address
echo "Replacing IP placeholder with current IP address..."
...
wget -O /etc/haproxy/haproxy.cfg "https://raw.githubusercontent.com/ajaysams/All/main/ssh/haproxy.cfg"
...
<p style="text-align: center;">
    <span style="color: #41A85F; font-size: 26px;"><strong>DARKANON TUNNELING</strong></span>
</p>
<p style="text-align: center;">
    <span style="font-family: 'Trebuchet MS', Helvetica, sans-serif;">
        <span style="color: #E25041; background-color: #61BD6D;">Your Favorite VPN</span>
        <span style="background-color: #61BD6D;">&nbsp;</span>
    </span>
</p>
<p style="text-align: center;">
    <span style="color: #B8312F;">WhatsApp</span>:
    <span style="color: #EFEFEF;">+254778216971</span>
</p>
END
